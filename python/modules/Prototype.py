...
def FilteredExcerptsMenuItem(excerpts:Iterable[dict], filter:Filter.Filter, formatter:Formatter, mainPageInfo:Html.PageInfo, menuTitle:str, fileExt:str = "", showDuration:bool = False) -> Html.PageDescriptorMenuItem:
    ...
    if showDuration:
        ...
        blankPage.AppendContent(Html.Tag("p")(durationStr))
...
def AllExcerpts(pageDir: str) -> Html.PageDescriptorMenuItem:
    ...
    def FilteredItem(filter:Filter,name:str) -> Html.PageDescriptorMenuItem:
        ...
        if name.endswith("ies"):
            singular = name.lower()[:-3] + "y"
        else:
            singular = name.lower().rstrip("s")
        return FilteredExcerptsMenuItem(excerpts,filter,formatter,pageInfo._replace(title=newTitle),name,singular,showDuration=True)

    excerpts = gDatabase["excerpts"]
    filterMenu = [
        FilteredExcerptsMenuItem(excerpts,Filter.PassAll,formatter,pageInfo,"All excerpts",showDuration=True),
    ...
    """FilteredExcerptsMenuItem(excerpts,Filter.Kind(category="Questions"),formatter,pageInfo._replace(title="All questions"),"Questions","question"),
...
def EventsMenu(indexDir: str) -> Html.PageDescriptorMenuItem:
    ...
    yield from baseTagPage.AddMenuAndYieldPages(eventMenu,**SUBMENU_STYLE)
...
def TeacherMenu(indexDir: str) -> Html.PageDescriptorMenuItem:
    ...
    baseTagPage = Html.PageDesc()
    yield from baseTagPage.AddMenuAndYieldPages(teacherMenu,**SUBMENU_STYLE)
