...
def FilteredExcerptsMenuItem(excerpts:list[dict], filter:Filter, formatter:Formatter, mainPageInfo:PageDesc.PageInfo, menuTitle:str, fileExt:str = ""):
...
def TagPages(tagPageDir: str) -> Iterator[PageDesc.PageAugmentorType]:
    ...
    for tag,tagInfo in gDatabase["tag"].items():
        ...
        # quotes = list(Filter.Apply(gDatabase["excerpts"],Filter.Tag(tag,category="Quotes")))
        ...
        quoteInfo = pageInfo._replace(title="Quotes",file=Utils.AppendToFilename(pageInfo.file,"-quotes"))
        basePage = PageDesc.PageDesc(pageInfo)
        basePage.AppendContent(str(a))
        ...
        #filterMenu.append(itertools.chain([pageInfo._replace(title="All")],MultiPageExcerptList(blankPage,relevantQs,formatter)))
        #filterMenu.append(itertools.chain([quoteInfo],MultiPageExcerptList(blankPage,quotes,formatter)))
